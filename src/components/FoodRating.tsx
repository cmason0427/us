...
/**
 * Each of you scores it 1-10 (either of you can set or fix both rows, since
 * one of you usually knows). Tap the same number again to clear it.
 */
export function FoodRating({ kind, id }: { kind: "place" | "meal"; id: string }) {
  ...
  async function rate(who: string, n: number) {
    const supabase = supabaseBrowser();
    const cur = ratings.find((r) => r.user_id === who)?.score;
    const { error } =
      cur === n
        ? await supabase.from("food_ratings").delete().eq("kind", kind).eq("ref_id", id).eq("user_id", who)
        : await supabase.from("food_ratings").upsert({ kind, ref_id: id, user_id: who, score: n, updated_at: new Date().toISOString() });
      ...
      {people.map((p) => {
        ...
        return (
          <div key={p.id} className="food-rating-row">
            ...
            <div className="score-row" role="group" aria-label={`${isMe ? "Your" : `${p.display_name}'s`} score`}>
              {Array.from({ length: 10 }, (_, i) => i + 1).map((n) => (
                <button key={n} type="button" aria-pressed={score != null && n <= score} className={score === n ? "on" : ""} onClick={() => rate(p.id, n)} aria-label={`${n} out of 10`}>
                  {n}
                </button>
              ))}
            </div>
            <span className="small food-face">{score ? face(score) : ""}</span>
          </div>
        );
      })}
    </div>
